...
let KmcInteraction = {
    ...
    openWebPageByNative: function (type, url, functionCode, userDataJson) {
        ...
        window.AlipayJSBridge && window.AlipayJSBridge.call(
            ...
            function (result) {
                if (result) {
                    console.log("h5:openWebPageByNative:return=>" + JSON.stringify(result));
    ...
    showKeyBoard: function (id, type, handleKeyBoardClick, getY) {
        ...
        window.AlipayJSBridge && window.AlipayJSBridge.call(
            ...
            function (result) {
                ...
                if (result) {
                    console.log("h5:showKeyBoard:return=>" + JSON.stringify(result));
    ...
    setHistoricalLoginAccount: function (jsonStr) {
        console.log(['setHistoricalLoginAccount', jsonStr].join(","));
    ...
    callbackAppTradeFunction: function (functionKey, jsonStr) {
        console.log(['callbackAppTradeFunction', functionKey, jsonStr].join(","));
    ...
    jumpToNativeInterface: function (isCloseCurrent, pageKey, paramJson) {
        console.log(['jumpToNativeInterface', isCloseCurrent, pageKey, paramJson].join(', '));
    ...
    openThirdPartyWebInterface: function (closeCurrent, url) {
        console.log(['openThirdPartyWebInterface', closeCurrent, url].join(', '));
    ...
    loginToLocalWeb: function (url, type, json = '{}') {
        console.log(['loginToLocalWeb', url, type, json].join(', '));
        if (KmcEnv.platform.isPhone) {
