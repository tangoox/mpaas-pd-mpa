...
export default {
    ...
    set: async function (key, value, both = false) {
        ...
        if (window.AlipayJSBridge) {
            console.log('缓存通过window.AlipayJSBridge写入Ap');
    ...
    remove: function (key) {
        ...
        if (window.AlipayJSBridge) {
            kmcLocalStorage.setLocalData(key, null, (KmcEnv.platform.isPhone ? 1 : 0))
        }
    ...
    objectification: function (key, dataStr) {
        // if (this.isEncryptedField(key) && !bd_utils.isEmpty(dataStr)) {
        //     dataStr = kmcLocalStorage.decryptData(dataStr);
        // }
        try {
